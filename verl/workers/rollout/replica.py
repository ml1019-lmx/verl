...
def get_rollout_replica_class(rollout: str, disaggregation_enabled: bool = False) -> type[RolloutReplica]:
    ...
    PD-disaggregated backends reuse their normal backend names; the dispatch
    here picks the PD replica only when the caller asserts
    ``disaggregation_enabled=True`` (sourced from
    ``RolloutConfig.disaggregation.enabled``).
    """
    if disaggregation_enabled:
        ...
        raise NotImplementedError(
            f"PD disaggregation is only supported with rollout='sglang' or rollout='vllm'; got {rollout!r}."
        )
